...
public class App {
	public static void main(String[] args) {
		...
		List<Author> authors = Arrays.asList(author2, author3, author4);
		List<Author> authors1 = Arrays.asList(author1);
		...
		books.addAll(Arrays.asList(book, book1, book2, book3, book4));
		/// 2.3.1
		...
		books.stream().filter(Objects::nonNull).map(Book::getAuthors).filter(Objects::nonNull)
				.filter(author -> author.size() == 1).forEach(System.out::println);
		/// 2.3.5 and 2.3.6
		...
		books.stream().map(Book::getAuthors).filter(Objects::nonNull).flatMap(list -> list.stream()).distinct().forEach(System.out::println);
